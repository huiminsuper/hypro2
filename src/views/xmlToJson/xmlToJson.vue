...
<script>
	...
	export default {
		data () {
			return {
				// xmlJson: '<Workflow xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema" encoding="GB2312"><Nodes title="張三"><NodeId>2018_11_05</NodeId><NodeType>Task</NodeType></Nodes></Workflow>'
				xmlJson: undefined
			}
		},
		created () {
			...
			xml () {
				$.ajax({
					...
					success: (resp) => {
						...
						for(let i = 0; i < resp.childNodes.length; i++) {
							console.log(resp.childNodes[i]);
						}
						// console.log(obj);
						console.log(resp);
						console.log(this.xmlToJson(resp))
						
// 						this.xmlJson = $.xml2json(resp)
// 						console.log($.xml2json(resp));
					}
				})
			},
			xmlToJson(xml) {
				...
				if(xml.childNodes.length != 0 && xml.childNodes.length != null) {
					...
					for(let i = 0; i < xml.childNodes.length; i++) {
						...
						if(x.nodeType != 1) {
							let value_text = x.nodeValue.replace('/\s*$|^\s*/g', "");
							if(value_text != "" && value_text != null && /^[\,:{}\s]*$/.test(value_text.replace(/\\(?:["\\\/bfnrt]|u[0-9a-fA-F]{4})/g, "@")
							.replace(/"[^"\\\n\r]*"|true|false|null|-?\d+(?:\.\d*)?(?:[eE][+\-]?\d+)?/g,"]")
							.replace(/(?:^|:|,)(?:\s*\[)+/g, ""))){
								// /^[\,:{}\s]*$/.test(value_text.replace(/\\(?:["\\\/bfnrt]|u[0-9a-fA-F]{4})/g, "@").replace(/"[^"\\\n\r]*"|true|false|null|-?\d+(?:\.\d*)?(?:[eE][+\-]?\d+)?/g,"]").replace(/(?:^|:|,)
						...
						}else if(text!=null && text!=""&&/^[\,:{}\s]*$/.test(value_text.replace(/\\(?:["\\\/bfnrt]|u[0-9a-fA-F]{4})/g, "@")
							.replace(/"[^"\\\n\r]*"|true|false|null|-?\d+(?:\.\d*)?(?:[eE][+\-]?\d+)?/g,"]")
							.replace(/(?:^|:|,)(?:\s*\[)+/g, ""))) {
